$(() => {
    ...
    let menulevel2 = [["占位符", "占位符", "占位符"], ["占位符", "占位符", "占位符"], ["占位符", "占位符", "占位符"], ["占位符", "占位符", "占位符"]];
    for (const key in menulevel2) {
        if (Object.hasOwnProperty.call(menulevel2, key)) {
            ...
            $("header .menu .menu" + key).mouseenter(function () { $(this).find("ul").stop().slideDown(1000); });//绑定事件，鼠标悬停展开
            $("header .menu .menu" + key).mouseleave(function () { $(this).find("ul").stop().slideUp(1000); });//鼠标离开升起
            for (let j = 0; j < element.length; j++) {
                $("header .menu .menu" + key + " ul").append("<li><a href=\"javascript:void(0)\">" + element[j] + "</a></li>");
            }
        }
    }
