$(() => {
    ...
    $.ajax({
        ...
        success: function (data) {
            let menulevel2 = JSON.parse(data);
            ...
            $(".menu div ul li").on("click", function () {
                ...
                $("#intro").slideUp(1000, () => {
                    $("#intro span").text(headline);
                    $.ajax({
                        type: "POST",
                        url: "articles.jsp",
                        data: { queryType: "DETAIL", headline: headline },
                        dataType: "text",
                        success: function (data) {
                            $("#intro div").html(marked.parse(data));
    ...
    $("#fontSizeSwitcher .fontSize:eq(0)").on("click", () => {//字体小
        ...
        $("#intro span").animate({ "font-size": "2em" }, 500);

        $("#intro div p").animate({ "font-size": "18px", "text-indent": "36px" }, 500);
    })
    $("#fontSizeSwitcher .fontSize:eq(1)").on("click", () => {//字体中
        $("#sliding_block").animate({ "left": "35%" }, 500);
        $("#intro span").animate({ "font-size": "3em" }, 500);
        $("#intro div p").animate({ "font-size": "22px", "text-indent": "44px" }, 500);
    })
    $("#fontSizeSwitcher .fontSize:eq(2)").on("click", () => {//字体大
        $("#sliding_block").animate({ "left": "68%" }, 500);
        $("#intro span").animate({ "font-size": "4em" }, 500);
        $("#intro div p").animate({ "font-size": "28px", "text-indent": "56px" }, 500);
    })
